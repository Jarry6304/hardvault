...
- `hardvault verify --decrypt`：真實解密驗證每筆密文，並比對明文 config bytes，
  也會偵測 `.cs` 內有但 `secrets.toml` 沒有的 stale KEY
- `src/parse_cs.rs` 模組：解析 codegen 產出的 `.cs`，提取 entries
  （供 `verify --decrypt` 使用）
- CI 加入 `cargo-audit` job，掃描 CVE 公告
- `.github/dependabot.yml`：weekly 更新 Rust 與 GitHub Actions 依賴
- `CHANGELOG.md`：本檔
- `CONTRIBUTING.md`：給 GitHub UI 自動偵測的快速貢獻入口

### Tests

- +11 unit tests for `parse_cs`
- +4 integration tests for `verify --decrypt`（含 wrong key、stale .cs、extra key 情境）
- 總計 **61 tests**（41 unit + 17 integration + 3 roundtrip）
...
- `.github/workflows/ci.yml`：rust-lint、rust-test matrix（ubuntu/macos/windows）、
  dotnet-sample e2e（含解密 + 失敗驗證）、all-checks-pass gate
